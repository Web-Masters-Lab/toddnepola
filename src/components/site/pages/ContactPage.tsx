          ...
          <div data-c-grid style={css('padding:128px 0 128px;margin:0 auto;max-width:80rem;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));')}>
            <div data-c-col style={css('padding:0 32px;position:relative;z-index:1;')}>
              <div style={css('margin:0 auto;max-width:32rem;')}>
                <h2 style={css("font-size:clamp(40px,7vw,86px);text-align:center;font-weight:700;letter-spacing:-.025em;text-transform:uppercase;text-wrap:balance;font-family:'Roboto Condensed',sans-serif;color:#fff;margin:0;")}>Get in Touch</h2>

                {sent ? (
                  <div style={css('margin-top:56px;text-align:center;color:#fff;')}>
                  ...
                  <form data-c-form method="POST" name="contact" onSubmit={handleSubmit} style={css('margin-top:64px;')}>
                    ...
                    <div data-c-fields style={css('display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:24px 32px;')}>
                      <div>
                        <label style={css(labelStyle)}>First name</label>
                        <div style={css('margin-top:10px;')}><input type="text" name="first-name" autoComplete="given-name" required style={css(inputStyle)} /></div>
                      </div>
                      <div>
                        <label style={css(labelStyle)}>Last name</label>
                        <div style={css('margin-top:10px;')}><input type="text" name="last-name" autoComplete="family-name" style={css(inputStyle)} /></div>
                      </div>
                      <div style={css('grid-column:span 2 / span 2;')}>
                        <label style={css(labelStyle)}>Email</label>
                        <div style={css('margin-top:10px;')}><input type="email" name="email" autoComplete="email" required style={css(inputStyle)} /></div>
                      </div>
                      <div style={css('grid-column:span 2 / span 2;')}>
                        <div style={css('display:flex;justify-content:space-between;font-size:14px;line-height:1.5;')}>
                          <label style={css(labelStyle)}>Phone</label>
                          <p style={css(noteStyle)}>Optional</p>
                        </div>
                        <div style={css('margin-top:10px;')}><input type="tel" name="phone" autoComplete="tel" style={css(inputStyle)} /></div>
                      </div>
                      <div style={css('grid-column:span 2 / span 2;')}>
                        <div style={css('display:flex;justify-content:space-between;font-size:14px;line-height:1.5;')}>
                          <label style={css(labelStyle)}>Message</label>
                          <p style={css(noteStyle)}>Max 500 characters</p>
                        </div>
                        <div style={css('margin-top:10px;')}><textarea name="message" rows={4} maxLength={500} style={css("display:block;width:100%;border-radius:6px;border:0;padding:8px 14px;background:#f3f4f6;color:#000;box-shadow:inset 0 0 0 1px #d1d5db;font-size:14px;font-family:'Roboto',sans-serif;")} /></div>
                      </div>
                    </div>
